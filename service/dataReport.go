...
// Create 创建数据报告
func (s *DataReportService) Create(report *models.DataReportCreateReq) (*models.DataReport, error) {
	if report.Name == "" {
...
// Update 更新数据报告
func (s *DataReportService) Update(report *models.DataReportUpdateReq) (*models.DataReport, error) {
	if report.ID == 0 {
...
// UpdateStatus 更新报告状态
func (s *DataReportService) UpdateStatus(id int64, status int8) error {
	if id <= 0 {
		return errors.New("无效的ID")
	}
